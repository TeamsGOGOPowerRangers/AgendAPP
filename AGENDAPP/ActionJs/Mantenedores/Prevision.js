...
window.addEventListener("load", function () {
    ...
    xhr.open('POST', '../Mantenedores/PrevisionLitas', true);
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (resultado.RESPUESTA) {
                ...
                var table = document.getElementById('tabla_Prevision');
                var tbody = table.getElementsByTagName('tbody')[0];
                for (var i = 0; i < resultado.datos.length; i++) {
                    var cuerpo = '<tr>' +
                        '<td>' + resultado.datos[i].NOMBRE_PREVISION + '</td>' +
                    ...
                    cuerpo += '<td>';
                    cuerpo += '<button class="btn btn-outline-primary btn-sm" id="ModificarPrevision" value=' + resultado.datos[i].ID_PREVISION + ' data-container="body" data-toggle="popover" data-trigger="hover" data-placement="top" data-content="Modificar Prevision"><i class="fa fa-edit"></i></button>';
                    cuerpo += '</td>' +
    ...
    $("#AddPrevisiones").click(function () {
        $("#mdlCrear").modal("show");
    });
});

$("#BtnGuardarPrevision").click(function (e) {
    e.preventDefault();
    var Errores = 0;
    $("#tabla_Prevision").dataTable().fnDestroy();
    var datos = new FormData();

    var NombrePrevision = $("#txtNombre").val();
    var Descripcion = $("#txtDescripcion").val();
    datos.append("NombrePrevision", NombrePrevision);
    ...
    if (Errores === 0) {
        $.ajax({
            ...
            //dataType: 'json',
            type: 'POST',
            url: '../Mantenedores/GuardarPrevision',
            success: function (d) {
                if (d.RESPUESTA) {
                    ...
                    Boton.value = d.data.ID_PREVISION;
                    Boton.id = 'ModificarPrevision';
                    ...
                    Boton.setAttribute('data-content', 'Modificar Prevision');
                    ...
                    Celda.innerText = d.data.NOMBRE_PREVISION;
...
$(document).on('click', '#ModificarPrevision', function (e) {
    e.preventDefault();
    var ID = $(this).val();
    $.ajax({
        type: 'POST',
        url: '../Mantenedores/ListaPrevisionId',
        data: { 'id': ID },
        success: function (d) {
            if (d.RESPUESTA) {
                ...
                $("#BtnModificarPrevision").val(datos[0].ID_PREVISION);
                $("#txtMNombre").val(datos[0].NOMBRE_PREVISION);
...
$(document).on('click', '#BtnModificarPrevision', function (e) {
    e.preventDefault();
    var Errores = 0;
    var datos = new FormData();

    var ID_PREVISION = $(this).val();
    var NOMBRE = $("#txtMNombre").val();
    var DESCRIPCION = $("#txtMDescripcion").val();
    var COD_ESTADO = $("#MEstado").val();

    datos.append("ID_PREVISION", ID_PREVISION);
    ...
    if (NOMBRE === "") {
        Errores += 1;
        new Noty({
            text: '<strong>Información</strong><br /> Debe ingresar el nombre de Prevision.',
            type: 'error',
            theme: 'sunset',
            layout: 'topRight',
            timeout: 4000,
    ...
    if (Errores === 0) {
        ...
        $.ajax({
            ...
            contentType: false,
            type: 'POST',
            url: '../Mantenedores/ModificarPrevision',
            success: function (d) {
                if (d.RESPUESTA) {
                    new Noty({
                        text: '<strong>Información</strong><br /> Guardado!.',
                    ...
                    for (var i = 0; i < tbody.getElementsByTagName('tr').length; i++) {
                        ...
                        if (Number(btun.value) === d.data.ID_PREVISION) {

                            fila.getElementsByTagName('td')[0].innerText = d.data.NOMBRE_PREVISION;
